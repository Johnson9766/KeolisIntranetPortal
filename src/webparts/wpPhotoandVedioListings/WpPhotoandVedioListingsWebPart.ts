...
export default class WpPhotoandVedioListingsWebPart extends BaseClientSideWebPart<IWpPhotoandVedioListingsWebPartProps> {
  ...
  public render(): void {
    ...
    try {
      ...
      photoVideoListings.html = photoVideoListings.html.replace(new RegExp("__KEY_URL_RESOURCE__", 'g'),this._ResourceUrl);
  ...
  private async _getLibraryFiles(): Promise<ISPList[]> {
    ...
    try {
      const apiUrl = `${this.context.pageContext.web.absoluteUrl}/_api/web/lists/getbytitle('${this.listName}')/items?$select=FileLeafRef,File/ServerRelativeUrl&$expand=File&$orderby=Created desc`;
