...
import homeWidgetsHtml from './homeWidgetsHtml';

import { SPHttpClient, SPHttpClientResponse } from '@microsoft/sp-http';
import {SPComponentLoader} from '@microsoft/sp-loader'


// import styles from './WpHomeWidgetsWebPart.module.scss';
import * as strings from 'WpHomeWidgetsWebPartStrings';

import GlobalImageExtensions from '../GlobalImageExtensions';
import GlobalVideoExtensions from '../GlobalVideoExtensions';

export interface IWpHomeWidgetsWebPartProps {
  description: string;
}
...
export default class WpHomeWidgetsWebPart extends BaseClientSideWebPart<IWpHomeWidgetsWebPartProps> {
 ...
 // private _isDarkTheme: boolean = false;
  ...
  private siteName: string = 'KeolisIntranetDev';


  public async render(): Promise <void>  {

    homeWidgetsHtml.allElementsHtml = homeWidgetsHtml.allElementsHtml.replace(/__KEY_URL_RESOURCE__/g,this._ResourceUrl); 
    this.domElement.innerHTML = homeWidgetsHtml.allElementsHtml;
    this.loadCSS();
  ...
  private _renderAnnoList(items: ISPList[]): void {
    ...
    try {
      ...
      console.log(fileName);
        ...
        let singlehomeWidgetsHtmlElement = '';
        ...
        // if (GlobalImageExtensions.AllImageExtensions.some(imageExt => imageExt.ImageExtension === fileExtension)) {
          ...
          singleSliderHTMLElement = homeWidgetsHtml.singleAnnouncementSliderElementHtml;
        // } else if (GlobalVideoExtensions.AllVideoExtensions.some(videoExt => videoExt.VideoExtension === fileExtension)) {
      
        //   singleHTMLElement = homeWidgetsHtml.singleAnnouncementVideoDesc;
        // }
      ...
      singlehomeWidgetsHtmlElement = homeWidgetsHtml.singleAnnouncementListElementHtml;

      singlehomeWidgetsHtmlElement = singlehomeWidgetsHtmlElement
      .replace("__KEY_DATA_TITLE__",item.Title)
      .replace("__KEY_DATA_DESC__",item.Description)
      .replace("__KEY_URL_IMG__",fileName);

      allElementsAnnHtml += singlehomeWidgetsHtmlElement;
    ...
    if (allElementsSliderHtml == "") {
      allElementsSliderHtml = homeWidgetsHtml.noRecord; }

      if(allElementsAnnHtml == ""){
        allElementsAnnHtml = homeWidgetsHtml.noRecord;
      }
   ...
   private _renderQuickLinks(items: ISPList[]): void {
    ...
    try {
      ...
      console.log(fileName);
        ...
        singleQuickLinkHTMLElement = homeWidgetsHtml.singleQuickLinkElement;
    ...
    // if no announcements, then display no record msg
    if (allElementsQuickLinksHtml == "") { allElementsQuickLinksHtml = homeWidgetsHtml.noRecord; }
  ...
  private _renderNewsList(items: ISPList[]): void {
    ...
    try {
      ...
      console.log(fileName);
        ...
        singleNewsCentreElement = homeWidgetsHtml.singleNewsCentreElement;
    ...
    // if no announcements, then display no record msg
    if (allElementsNewsCenterHtml == "") { allElementsNewsCenterHtml = homeWidgetsHtml.noRecord; }
  ...
  private _renderPhotosVideosList(items: ISPList[]): void {
   let allPhotosVideosElements = '';
    ...
    // if no announcements, then display no record msg
    if (allPhotosVideosElements == "") { allPhotosVideosElements = homeWidgetsHtml.noRecord; }
...
private getMonth(date: Date): string {
  ...
  private _renderUpcomingEventsList(items: ISPList[]): void {
    ...
    try {
      ...
      // console.log(fileName);
        ...
        singleUpcomingEventElement = homeWidgetsHtml.singleUpcomingEventElement;
    ...
    // if no announcements, then display no record msg
    if (allElementsUpcomingEventsHtml == "") { allElementsUpcomingEventsHtml = homeWidgetsHtml.noRecord; }
  ...
  private loadHome():void{
    ...
    SPComponentLoader.loadScript(`/sites/KeolisIntranetDev/SiteAssets/resources/js/common.js`);
    SPComponentLoader.loadScript(`/sites/KeolisIntranetDev/SiteAssets/resources/js/home.js`);        
  }
